...
We defined *how* a fake Post should look.
...
*   **Factories are the "What":** They define the shape of your data. A factory knows that a "User" has a name, an email, and a password. It doesn't know *how many* users you want, just what one looks like.
...
1.  **Relationship Magic:**
    ...
    -   *Good:* `Post::factory()->count(10)->create()` will create 10 posts AND 10 users (one for each).
    -   *Better (for performance):* Create a user first, then attach posts.
        ```php
